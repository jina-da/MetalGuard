...
└── server/
    ...
    │   └── mfc_handler.py     # MFC 판정 결과 전송 / 재분류 수신
    ├── engine/
    │   ├── verdict_engine.py  # Queue 소비 → 추론 → 판정 → 전송
    │   ├── arduino_serial.py  # 아두이노 시리얼 통신
    │   └── timeout_handler.py # 타임아웃 처리 (추후 구현 예정)
    └── db/
...
| RECLASSIFY_CONFIRM | 401 | MFC → 운용서버 | 재분류 확정 |
...
| `reclassify_result` | UNCERTAIN 재분류 결과 |
